...
) 
{
  ...
  arm_q7_to_q15_reordered_no_shift(pInBuffer, pInBuffer_15, dim_sequence*heads*dim_sequence);
  ...
  {  
    for (seq_out = 0; seq_out < dim_sequence; seq_out++)
    {
      ...
      pOut = pOutBuffer + head_out * projections + seq_out * heads * projections;
      ...
      {  
        ...
        pA = pInBuffer_15 + head_out * dim_sequence + seq_out * heads * dim_sequence;

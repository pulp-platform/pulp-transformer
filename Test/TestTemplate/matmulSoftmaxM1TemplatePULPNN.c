...
void cluster_fork(void *args) {
  ...
  int8_t *A_reshape_buffer = O + ${outputSize};
  int8_t *B_reshape_buffer = A_reshape_buffer + ${sizeA};
  int8_t *softmax_buffer = B_reshape_buffer + ${sizeB};

  // Reshape Q and K, from (S,PH) -> (H,S,P)
  for(int h = 0; h < ${H}; h++){
    for(int s = 0; s < ${S}; s++){
      for(int p = 0; p < ${P}; p++){
        A_reshape_buffer[h*${S}*${P} + s*${P} + p] = A_copy[s*${P}*${H} + h*${P} + p];
        B_reshape_buffer[h*${S}*${P} + s*${P} + p] = B_copy[s*${P}*${H} + h*${P} + p];
      }
    }
  }

  // Transpose K, from (H,S,P) -> (H,P,S)
  // for(int h = 0; h < ${H}; h++){
  //   for(int s = 0; s < ${S}; s++){
  //     for(int p = 0; p < ${P}; p++){
  //       B_copy[h*${S}*${P} + p*${S} + s] = B_reshape_buffer[h*${S}*${P} + s*${P} + p];
  //     }
  //   }
  // }
  A = A_reshape_buffer;
  ...
  //     ${kernelName}(A_copy, NULL, O_copy, B_copy, NULL, NULL, ${requantMul}, ${requantDiv}, ${P}, ${S}, 1, 0);
  //   }
  // }
  // pi_cl_team_barrier(0);
  ...
  //     O_copy = O + h*${S}*${S} + s*${S};
  //     A_copy = A + h*${S}*${S} + s*${S};
  //     iSoftmax(O_copy, A_copy, ${S}, 1, 7, 24, 5, 256);
  //   }
  // }
  // pi_cl_team_barrier(0);
  // STOP_PROFILING(Sequential);
  ...
  for(int h = 0; h < ${H}; h++){
    for(int s = 0; s < ${S}; s++){
      ...
      O_copy = O + h*${S}*${S} + s*${S};
      pi_cl_team_barrier(0);
      ${kernelName}(A_copy, NULL, softmax_buffer, B_copy, NULL, NULL, ${requantMul}, ${requantDiv}, ${P}, ${S}, 1, 0);
      pi_cl_team_barrier(0);
      pi_cl_team_barrier(0);
      START_PROFILING();

/* ----------------------------------------------------------------------
#
# File: matmulSoftmaxFWATemplate.c
#
...
void cluster_fork(void *args) {
  ...
  int P = ((int *)args)[6];
  int H = ((int *)args)[7];
  int preProjRequantDiv = ((int *)args)[8];
  int preProjRequantMul = ((int *)args)[9];
  int postProjRequantDiv = ((int *)args)[10];
  int postProjRequantMul = ((int *)args)[11];
  ...
  for(int i = 0; i < ${numberOfTransferI}; i++){
    DMA_copy_I.ext = (int)&(${vectorNameI}) + ${dmaTransferSize}*i;
    DMA_copy_I.loc = I + ${dmaTransferSize}*i;
    thorir_dma(DMA_copy_I);
    pi_cl_team_barrier(0); 
  }

  for(int i = 0; i < ${numberOfTransferW}; i++){
    DMA_copy_W.ext = (int)&(${vectorNameW}) + ${dmaTransferSize}*i;
  ...
  #ifdef TEST_INPUTS
    if (pi_core_id()==0) {
      printf("Tensor I check:\n");
      int checkA = 1;
      for(int i = 0; i < ${sizeI}; i++){
        if((int8_t)I[i] != ${vectorNameI}[i]){
          printf("\nError: Tensor I Transfer Error at %d: %d / %d\n", i, I[i], ${vectorNameI}[i]);
          checkA = 0;
        }
      }
      if(checkA){
        printf("Tensor I Transfer Correct");
      }
      printf("\n");

      printf("Tensor W check:\n");
      int checkB = 1;
      for(int i = 0; i < ${sizeW}; i++){
        if((int8_t)W[i] != ${vectorNameW}[i]){
          printf("\nError: Tensor W Transfer Error at %d: %d / %d\n", i, W[i], ${vectorNameW}[i]);
          checkB = 0;
        }
      }
      if(checkB){
        printf("Tensor W Transfer Correct");
      }
      printf("\n");
    }
  #endif
  ...
  pi_cl_team_barrier(0);
  START_PROFILING();
  ${kernelName}(I, W, B, O, ${S}, ${E}, ${H}, ${requantDiv}, ${requantMul}, ${requantDiv}, ${requantMul}, 1, 7, 24, 5, 256);
  pi_cl_team_barrier(0);
...
void kernel_task(void *task_args) {
  ...
  char *I = (char *) (L1_buffer + ${dmaTransferSize});
  char *W = (char *) (L1_buffer + ${dmaTransferSize} + ${int(sizeI + dmaTransferSize)});
  char *B = (char *) (L1_buffer + ${dmaTransferSize} + ${int(sizeI + dmaTransferSize + sizeW + dmaTransferSize)});
  char *O = (char *) (L1_buffer + ${dmaTransferSize} + ${int(sizeI + dmaTransferSize + sizeW + dmaTransferSize + biasSize + dmaTransferSize)});
  printf("DONE\n");

/* ----------------------------------------------------------------------
#
# File: linearProjOTemplatePULPNN.c
#
...
void cluster_fork(void *args) {
  ...
  pi_cl_team_barrier(0);
  STOP_PROFILING(DMA Transfer Input);
  ...
  // Reshape A, from (H,S,P) -> (S,PH)
  for(int h = 0; h < ${H}; h++){
    for(int s = 0; s < ${S}; s++){
      for(int p = 0; p < ${P}; p++){
        A_reshape_buffer[s*${P}*${H} + h*${P} + p] = I_copy[h*${S}*${P} + s*${P} + p];
      }
    }
  }
  I = A_reshape_buffer;
  STOP_PROFILING(Transposition);
  pi_cl_team_barrier(0);
  START_PROFILING();
  for(int i = 0; i < ${S}; i++){
    ...
    ${kernelName}(I_copy, B_copy, O_copy, W_copy, NULL, NULL, ${requantMul}, ${requantDiv}, ${P*H}, ${E}, 1, 0);

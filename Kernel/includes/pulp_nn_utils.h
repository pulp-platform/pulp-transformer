...
static int8_t __attribute__ ((noinline))  pulp_nn_requantshift_i8_i8(
   int32_t Im_in, 
   int32_t mul, 
   int32_t add, 
   int32_t div
)
{
    ...
    intermediate = Im_in * mul;
    intermediate = intermediate + add;
    Im_out = (int8_t)clips8((int32_t)(intermediate >> div));
    return Im_out;
}
...
static int8_t __attribute__((noinline)) pulp_nn_bn_quant_i8 (
  ...
  int32_t integer_image_phi = (k * phi) + lambda;
  //int32_t x = (integer_image_phi) >> d;
  int8_t res = clips8(integer_image_phi >> d);
  return res;
}

    ...
    for (int s = 0; s < dim_sequence; s++){
      ...
      for (int e0 = 0; e0 < dim_embedding; e0++){
        ...
        for (int e1 = 0; e1 < (dim_embedding >> 2); e1++){
          vecI = *((v4s*) pInBuffer);
          vecW = *((v4s*) pWeight);
          accumulator = SumDotp(vecI, vecW, accumulator);
          pInBuffer += 4;
          pWeight += 4;
        }
    ...
    for (int s0 = 0; s0 < dim_sequence; s0++){
      ...
      for (int s1 = 0; s1 < dim_sequence; s1++){
        ...
        for (int e = 0; e < (dim_embedding >> 2); e++){
          vecI = *((v4s*) pInBuffer);
          vecW = *((v4s*) intermediateBuffer);
          accumulator = SumDotp(vecW, vecI, accumulator);
          intermediateBuffer += 4;
          pInBuffer += 4;
        }

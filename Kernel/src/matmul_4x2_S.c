  ...
  {  
    for (seq_out = start_seq; seq_out < stop_seq; seq_out++)
    {
      ...
      while(proj_out > 0){
        ...
        int seq_internal_left2 = dim_sequence % 4;
        while(seq_internal_left2>0){
          sum = sum + ((*pA++)) * (*pB);
          sum5 = sum5 + ((*pA2++)) * (*pB++);

          seq_internal_left2 -= 1;
        }
    ...
    if (seq_out_left){  
      ...
      {  
        ...
        int seq_internal_left3 = dim_sequence % 4;
      
        while(seq_internal_left3>0){
          sum = sum + ((*pA)) * (*pB++);
          sum2 = sum2 + ((*pA)) * (*pB2++);
          sum3 = sum3 + ((*pA)) * (*pB3++);
          sum4 = sum4 + ((*pA++)) * (*pB4++);

          seq_internal_left3 -= 1;
        }
        *pOut = clip8((sum*requant_mul)>>requant_div);
        pOut++;
        *pOut = clip8((sum2*requant_mul)>>requant_div);
      ...
      while(proj_out > 0){
        ...
        int seq_internal_left4 = dim_sequence % 4;

        while(seq_internal_left4>0){
          sum = sum + ((*pA++)) * (*pB++);
          seq_internal_left4 -= 1;
        }
